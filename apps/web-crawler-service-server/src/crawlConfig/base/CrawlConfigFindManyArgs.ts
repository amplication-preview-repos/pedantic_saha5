/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { CrawlConfigWhereInput } from "./CrawlConfigWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { CrawlConfigOrderByInput } from "./CrawlConfigOrderByInput";

@ArgsType()
class CrawlConfigFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => CrawlConfigWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => CrawlConfigWhereInput, { nullable: true })
  @Type(() => CrawlConfigWhereInput)
  where?: CrawlConfigWhereInput;

  @ApiProperty({
    required: false,
    type: [CrawlConfigOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [CrawlConfigOrderByInput], { nullable: true })
  @Type(() => CrawlConfigOrderByInput)
  orderBy?: Array<CrawlConfigOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { CrawlConfigFindManyArgs as CrawlConfigFindManyArgs };
